...
function add_friends_in_sidebar(friend) {
  ...
  chat_head.innerHTML = `
    ...
    <div class="text-container" id=${friend}>   
        <div class="text"><b>${friend}</b><div id="status" class="offline"></div><span><i class="fa fa-check" ></i>12:03 AM</span></div>
  ...
  chat_head.onclick = () => {
    var prev = receiver;
    if (prev) {
      socket.emit("not_typing", prev, uname);
    }
    if (receiver != friend) {
      receiver = friend;
...
function add_recent_messages_to_sidebar(
  ...
  time_place=document.querySelector(`#${temp} .text > span`)
  messaging_place=document.querySelector(`#${temp} > div:nth-child(2) > b`)
  if (type == "text") {
    time_place.innerHTML = time_
    messaging_place.innerHTML=msg.slice(0,21)+"..."
  } else if (type == "img") {
...
function online(friend) {
  stat = document.querySelector(`#${friend} #status`);
  stat.classList.remove("offline");
  stat.classList.add("online");
}
function offline(friend) {
  stat = document.querySelector(`#${friend} #status`);
...
socket.on("online", (friend) => {
  console.log("should be fucking online");
  online(friend);
});
socket.on("offline", (friend) => {
  console.log("should be fucking offline");
  offline(friend);
});
